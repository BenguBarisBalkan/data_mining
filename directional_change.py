...
def get_extremes(ohlc: pd.DataFrame, sigma: float):
    tops, bottoms = directional_change(ohlc['close'], ohlc['high'], ohlc['low'], sigma) # type: ignore
    ...
    extremes.to_csv('directional_change_results.csv')
    return extremes

if __name__ == '__main__':
    data = pd.read_csv('./data/ice_brent_1m.csv')
